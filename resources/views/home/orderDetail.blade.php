...
<main class="main-content">
    <div class="order-detail-container">
        {{-- Header Section: Trở lại và Mã đơn hàng | Trạng thái --}}
        <div class="order-header">
            <a href="javascript:history.back()" class="back-link"><i class="fas fa-arrow-left"></i> Trở lại</a>
        ...
        {{-- Customer Info Section: Địa chỉ, Tên người mua, Số điện thoại --}}
        <div class="customer-info-section">
            <h3>Thông tin khách hàng</h3>
            {{-- Đã thêm <span> bọc nhãn và sử dụng gap trên <p> --}}
                            ...
                            <tr class="warranty-details-row">
                                <td colspan="5">
                                    <div class="warranty-details-content">
                                        @if($item->productInstances->isNotEmpty())
                                            ...
                                            <ul>
                                                @foreach($item->productInstances as $instance)
                                                    <li>
                                                        Serial: <strong>{{ $instance->serial_number }}</strong>
                                        ...
                                        @else
                                            <strong><i class="fas fa-times-circle icon-no-warranty"></i> Sản phẩm không hỗ trợ bảo hành.</strong>
                                        @endif
                                    </div>
                                </td>
                            </tr>
                        @endif
                        {{-- KẾT THÚC PHẦN THÔNG TIN BẢO HÀNH --}}
        ...
        <div class="order-actions">
            @if ($order->status == 'Chờ Xử Lý')
                <a href="{{ route('user.cancelOrder', $order->id) }}" class="btn btn-danger"
                onclick="confirmation(event, 'Bạn có chắc muốn hủy đơn?', 'Bạn sẽ không thể hoàn tác hành động này!')">
